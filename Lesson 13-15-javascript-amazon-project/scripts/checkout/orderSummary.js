...
export function renderOrderSummary() {
  ...
  //takes the data and generate all the HTM... THIS CALLED VIEW
  cart.forEach((cartItem) => {
    const productId = cartItem.productId;


    const matchingProduct = getProduct(productId); //equal to get the product Id
  ...
  document.querySelectorAll('.js-delivery-option').forEach((element) => {
    element.addEventListener('click', () => {
      const { productId, deliveryOptionId } = element.dataset; //get those values out out of the data attribute Line 120, 121
